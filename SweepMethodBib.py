...
class putranformer:
    def Z_pu(bus_data, branch_data, Sbase, Vbase, nl):
        PerUnit = {'Zpu':[], 'Rpu':[], 'Xpu':[]}
        Zbase = ((Vbase*1000)**2)/(Sbase*1000) #ohms
        for i in range(nl):
            PerUnit['Rpu'].append(branch_data['R'][i]/Zbase)
            PerUnit['Xpu'].append(branch_data['Xl'][i]/Zbase)
            PerUnit['Zpu'].append(PerUnit['Rpu'][i] + 1j*PerUnit['Xpu'][i])
        return[PerUnit,Zbase]
...
class report_generation:
    def report(path,arquivo,ntd,ntn, nb, nl, convergence_report, Power_Flow, bus_data, branch_data, E, V_mod_pu, V_mod, theta_rad, theta_graus, Pk, Qk):
        ...
        arq.write('=='*60)
        arq.write('\n')
        arq.write('  Backward Forward Sweep Method\n')
        arq.write('  Convergence Tolerance: {:0.8f}\n'.format(float(E)))
        arq.write('--'*60) 
        arq.write('\n')
        arq.write('  Iteration  ')
        arq.write('  Mismatche  ')
